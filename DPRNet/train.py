...
# python -m tensorboard.main --logdir='tensorboard' --load_fast true
# --- Imports --- #
import time
import torch
import argparse
...
print("dataload_time: ")
print('({0:.0f}s)'.format(end_time_dataload-start_time_dataload))
...
print(end_time_val-start_time_val)
# old_val_psnr = 0
# old_val_ssim = 0
net.train()
#intializing GPStruct
#gp_struct = GPStruct(num_labeled, train_batch_size)
writer = SummaryWriter('./tensorboard/rain1200')
...
#-------------------------------------------------------------------------------------------------------------
    ...
    for batch_id, train_data in enumerate(lbl_train_data_loader):
        ...
        # loss = F.l1_loss(pred_image, gt)
        loss = ALPLoGLoss()(pred_image, gt)
